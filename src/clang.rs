...
impl Cursor {
    ...
    pub fn spelling(&self) -> String {
        unsafe { String_ { x: clang_getCursorSpelling(self.x) }.to_string() }
    }
    ...
    pub fn location(&self) -> SourceLocation {
        unsafe { SourceLocation { x: clang_getCursorLocation(self.x) } }
    }
    ...
    pub fn cur_type(&self) -> Type {
        unsafe { Type { x: clang_getCursorType(self.x) } }
    }

    pub fn definition(&self) -> Cursor {
        unsafe { Cursor { x: clang_getCursorDefinition(self.x) } }
    }
    ...
    pub fn enum_type(&self) -> Type {
        unsafe { Type { x: clang_getEnumDeclIntegerType(self.x) } }
    }
    ...
    pub fn typedef_type(&self) -> Type {
        unsafe { Type { x: clang_getTypedefDeclUnderlyingType(self.x) } }
    }
    ...
    pub fn ret_type(&self) -> Type {
        unsafe { Type { x: clang_getCursorResultType(self.x) } }
    }

    pub fn num_args(&self) -> i32 {
        unsafe { clang_Cursor_getNumArguments(self.x) }
    }
}
...
impl Type {
    ...
    pub fn declaration(&self) -> Cursor {
        unsafe { Cursor { x: clang_getTypeDeclaration(self.x) } }
    }

    pub fn is_const(&self) -> bool {
        unsafe { clang_isConstQualifiedType(self.x) == 1 }
    }
    ...
    pub fn canonical_type(&self) -> Type {
        unsafe { Type { x: clang_getCanonicalType(self.x) } }
    }

    // function
    pub fn is_variadic(&self) -> bool {
        unsafe { clang_isFunctionTypeVariadic(self.x) == 1 }
    }
    ...
    pub fn ret_type(&self) -> Type {
        unsafe { Type { x: clang_getResultType(self.x) } }
    }
...
impl File {
    pub fn name(&self) -> Option<String> {
        ...
        unsafe { Some(String_ { x: clang_getFileName(self.x) }.to_string()) }
    }
}
...
impl TranslationUnit {
    ...
    pub fn cursor(&self) -> Cursor {
        unsafe { Cursor { x: clang_getTranslationUnitCursor(self.x) } }
    }
...
impl Diagnostic {
    pub fn default_opts() -> CXDiagnosticDisplayOptions {
        unsafe { clang_defaultDiagnosticDisplayOptions() }
    }

    pub fn format(&self, opts: CXDiagnosticDisplayOptions) -> String {
        unsafe { String_ { x: clang_formatDiagnostic(self.x, opts) }.to_string() }
    }
